...
var layeredAnimation = (function() {
  ...
  function animateShape(el) {
    ...
    var animation = anime.timeline({
      targets: el,
      duration: function() { return anime.random(200, 2000); },
    ...
    .add({
      translateY: createKeyframes(function(el) {
        return el.classList.contains('large') ? anime.random(-300, 300) : anime.random(-400, 400);
      }),
    }, 0);
    if (circleEl) {
      animation.add({
        targets: circleEl,
        r: createKeyframes(function() { return anime.random(10, 72); }),
      }, 0);
    }
    if (rectEl) {
      animation.add({
